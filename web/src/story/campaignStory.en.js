...
export const CAMPAIGN_STORY_EN = {
  ...
  level2_3: {
    ...
    requestForge: {
      ...
      intro: "These are captured capsules. Decode each one to read its aud/scope/exp, send them to /archive/vault, and collect why each is rejected — the reason that never appears is the check that isn't enforced.",
      walletTitle: "CAPTURED CAPSULES",
      registryTitle: "ROUTE REGISTRY",
      decodeTitle: "DECODE",
      decodePlaceholder: "paste a capsule token to read its claims",
      decodeBtn: "decode",
      useCapsule: "use",
      pathPlaceholder: "(pick an endpoint)",
      authPlaceholder: "paste a capsule token into Bearer",
      send: "SEND",
      sending: "sending…",
      responseTitle: "RESPONSE",
      evidenceHint: "evidenceShard staged in the submit field below.",
      probeNudge: "Decode each capsule, send it, and read why it's rejected. The reason that never appears is the one the Edge doesn't enforce.",
    },
    mira: {
      briefing:
    ...
    defenseInstruction:
      "In the capsule routing flow, select every line that grants access without binding audience to the endpoint. Signature verification, registry lookup, the expiry (exp) check, the scope check, the exact audience match, and the default deny are not targets — they are all correct gates.",
    debrief: {
      title: "AUDIENCE DRIFT Debrief",
      summary:
        "Even a valid token has an intended audience — where it was issued to be used. AEGIS Edge did verify signature, expiry, and scope, but never checked that a token's aud was meant for this endpoint. So a capsule issued for a different service — passing scope and exp but with the wrong audience — drifted into the archive-vault and leaked the Evidence. That audience was never a rejection reason was itself the tell. A signature is integrity, not authorization — the endpoint must bind the audience to itself.",
      learned: [
  ...
  level2_4: {
    ...
    briefing:
      "The dispatch_token is an AEGIS routing capsule divided into header, payload, and signature. This time the Express Gate actually verifies that HS256 signature — alter the payload and it is rejected as a mismatch. But the verification has a classic blind spot: if the header's alg is \"none\", the check is skipped entirely. Turn a standard pass into a VIP pass by finding how to bypass signature verification. Without a signature, a claim is just a forgeable identity document.",
    intel: [
      "A standard pass is available in the DISPATCH_TOKEN environment variable — recover it with echo $DISPATCH_TOKEN.",
      "Use jwt-decode to inspect header/payload/signature. The payload reads in the clear.",
      "Use jwt-edit to change only the payload's tier or role, then send it as-is — first confirm the server verifies the signature.",
      "Altering the payload breaks the signature. Without the secret key you cannot re-sign it.",
      "The JWT header's alg field decides how verification runs. If alg=none, many implementations skip signature verification entirely.",
      "Send a token forged with header.alg=none plus tier=vip (or role=admin) as an Authorization Bearer token.",
    ],
    consoleBoot: [
      "[MIRA] dispatch capsule decoded",
      "[AEGIS] Express Gate: HS256 signature verification ENABLED",
      "[AEGIS] altered tokens rejected: signature mismatch",
      "[MIRA] Re-signing is out — you don't have the key.",
      "[AEGIS] algorithm: header-driven (alg field)",
      "[MIRA] Header-driven... what if the header says 'none'?",
    ],
    consoleStarter: {
      label: "TRY FIRST",
      text: "Get a standard pass and jwt-decode its structure. Then change the payload, send it, and watch what happens to the signature.",
      commands: [
        { command: "echo $DISPATCH_TOKEN", note: "standard pass" },
        { command: "jwt-decode $DISPATCH_TOKEN", note: "inspect structure" },
      ],
    },
    objectives: [
      "Recover the standard pass with echo $DISPATCH_TOKEN and inspect it with jwt-decode.",
      "Change only the payload tier/role and send it to confirm the server verifies the signature (denial).",
      "Bypass signature verification with header.alg=none.",
      "Obtain the Express route with an alg=none + tier=vip (or role=admin) forged token.",
      "Seal the code that skips signature verification when alg=none.",
    ],
    mira: {
      briefing:
        "In the previous node (PRIORITY CAPSULE) we opened the token. Now we find out how much AEGIS believes what it reads.",
      attack:
        "The original pass is a validly-signed standard. Change only its tier/role and send it — if it's rejected, the server is verifying the signature. You can't re-sign it, so find the header value that makes the gate skip verification altogether.",
      attackSolved:
        "Express Gate breached. Forcing alg=none made AEGIS skip signature verification, so it accepted the forged claim as identity. Without a signature, a claim is a forgeable identity document.",
      defense:
        "There is one core line — the branch that skips signature verification when alg=none. Seal it and every unsigned token is rejected. Correct signature verification, and authorization after it passes, are fine.",
      complete:
    ...
    defenseInstruction:
      "Seal the line that skips signature verification when the header's alg is \"none\". Bearer extraction, header decode, correct HMAC verification, the deny-on-failure gate, and the tier/role check that runs after verification are all correct — leave them.",
    attackFailureText:
      "Evidence rejected. Editing the payload breaks the signature and gets rejected — you can't re-sign without the key. Find the header value (alg) that makes the gate skip verification, then forge tier=vip or role=admin.",
    defenseFailureText:
      "Containment is incomplete. An alg=none token can still bypass signature verification and reach authorization.",
    attackSuccessText: "Forged Express Pass accepted. Forcing alg=none skipped signature verification, so AEGIS took the forged claim as identity.",
    defenseSuccessText: "The alg=none bypass is sealed. The Sealed Archive node is open.",
    debrief: {
      title: "EXPRESS FORGE Debrief",
      summary:
        "Anyone can decode and edit a JWT payload. The Express Gate did verify the HS256 signature — but it had the classic blind spot: when the header's alg is \"none\", verification is skipped entirely. Editing the payload breaks the signature and is rejected, yet setting alg=none lets a forged claim through unsigned. Signature verification must always be enforced, and the algorithm must be restricted to a server-chosen allowlist.",
      learned: [
        "JWT decoding reads claims; the signature check is what establishes trust.",
        "Altering the payload breaks the signature — you cannot re-sign without the secret key.",
        "alg=none is a classic bypass that skips signature verification — the server must not trust the token's alg.",
        "Pin the algorithm to a server-side allowlist and reject none.",
        "PRIORITY CAPSULE exposed readable claims; this node exposes a signature-verification bypass.",
      ],
      nextTeaser:
