...
export const CAMPAIGN_STORY = {
  ...
  level2_3: {
    ...
    requestForge: {
      ...
      intro: "포획한 캡슐들이야. 하나씩 decode해서 aud/scope/exp를 읽고, /archive/vault로 보내 거부 사유를 모아봐 — 안 나오는 사유가 곧 안 묶인 검사야.",
      walletTitle: "CAPTURED CAPSULES",
      registryTitle: "ROUTE REGISTRY",
      decodeTitle: "DECODE",
      decodePlaceholder: "캡슐 토큰을 붙여넣으면 claim이 보여",
      decodeBtn: "decode",
      useCapsule: "use",
      pathPlaceholder: "(endpoint를 골라)",
      authPlaceholder: "캡슐 토큰을 Bearer에 붙여넣어",
      send: "SEND",
      sending: "보내는 중…",
      responseTitle: "RESPONSE",
      evidenceHint: "evidenceShard가 아래 제출칸에 담겼어.",
      probeNudge: "각 캡슐을 decode하고 보내서 왜 거부되는지 읽어봐. 끝까지 거부 사유로 안 나오는 항목이 Edge가 안 묶은 검사야.",
    },
    mira: {
      briefing:
    ...
    defenseInstruction:
      "capsule을 endpoint로 라우팅하는 흐름에서 audience 바인딩 없이 접근을 허용하는 라인을 모두 골라 봉쇄해. 서명 검증·registry 조회·만료(exp) 검사·scope 검사·정확한 audience 일치·기본 거부는 대상이 아니다(모두 올바른 게이트).",
    debrief: {
      title: "AUDIENCE DRIFT 정리",
      summary:
        "유효한 토큰이라도 '어디에 쓰이도록 발급됐는가(audience)'가 정해져 있다. AEGIS Edge는 서명·만료(exp)·scope는 실제로 검증했지만, aud가 이 endpoint용인지 바인딩은 검증하지 않았다. 그래서 exp·scope는 통과하지만 다른 서비스용으로 발급된(audience가 틀린) 캡슐이 archive-vault로 drift해 Evidence를 꺼냈다. 거부 사유에 audience가 없다는 것 자체가 그 검사가 없다는 신호였다. 서명(무결성)은 곧 인가가 아니다 — 권한 확인은 endpoint가 스스로, audience를 자신에게 바인딩해서 해야 한다.",
      learned: [
  ...
  level2_4: {
    ...
    briefing:
      "PRIORITY CAPSULE에서 열어본 dispatch_token은 Header.Payload.Signature로 나뉜 AEGIS routing capsule이었다. 이번엔 Express Gate가 그 토큰을 어떻게 믿는지가 문제다. Gate는 이제 토큰의 HS256 서명을 실제로 검증한다 — payload를 바꾸면 서명이 깨져 거부된다. 하지만 서명 검증에는 고전적 맹점이 있다: header의 alg=none이면 검증을 통째로 건너뛴다. standard pass를 VIP로 위조하되, 서명 검증을 어떻게 우회하는지 찾아라. 서명이 없으면 claim은 위조 가능한 신분증이다.",
    progressiveHints: true,
    intel: [
      "터미널에 standard pass가 DISPATCH_TOKEN 환경 변수로 준비돼 있다. echo $DISPATCH_TOKEN으로 꺼낸다.",
      "jwt-decode로 header/payload/signature를 확인한다. payload는 평문으로 읽힌다.",
      "jwt-edit로 payload의 tier나 role만 바꿔 그대로 Express Gate에 보내본다 — 서버가 서명을 검증하는지 먼저 확인.",
      "payload를 바꾸면 서명이 깨진다. 비밀키가 없으니 재서명은 불가능하다.",
      "JWT header의 alg 값이 검증 방식을 정한다. alg=none이면 많은 구현이 서명 검증을 통째로 건너뛴다.",
      "header.alg=none + tier=vip(또는 role=admin)으로 위조한 토큰을 Bearer로 보낸다.",
    ],
    consoleBoot: [
    ...
    consoleStarter: {
      label: "TRY FIRST",
      text: "standard pass를 받아 jwt-decode로 구조를 봐. 그다음 payload를 바꿔 보내면 서명이 어떻게 되는지 직접 확인해봐.",
      commands: [
        { command: "echo $DISPATCH_TOKEN", note: "standard pass" },
        { command: "jwt-decode $DISPATCH_TOKEN", note: "구조 확인" },
      ],
    },
    objectives: [
      "echo $DISPATCH_TOKEN으로 standard pass를 꺼내 jwt-decode로 구조를 확인한다.",
      "payload의 tier/role만 바꿔 보내 서버가 서명을 검증함을 확인한다(거부).",
      "header.alg=none으로 서명 검증을 우회한다.",
      "alg=none + tier=vip(또는 role=admin) 위조 토큰으로 Express route를 획득한다.",
      "alg=none이면 서명 검증을 건너뛰는 코드를 봉쇄한다.",
    ],
    mira: {
      briefing:
        "앞 노드(PRIORITY CAPSULE)에서 토큰을 열어봤지. 이번엔 그 토큰을 AEGIS가 얼마나 믿는지 시험할 차례야.",
      attack:
        "원본 pass는 서명이 유효한 standard야. payload의 tier/role만 바꿔 보내봐 — 거부되면 서버가 서명을 검증한다는 뜻이야. 재서명은 비밀키가 있어야 하니, 검증 '자체'를 건너뛰게 만드는 header 값을 찾아.",
      attackSolved:
        "Express Gate 통과. alg=none으로 서명 검증을 건너뛰게 만드니, AEGIS는 위조된 claim을 그대로 신분으로 받아들였어. 서명이 없으면 claim은 위조 가능한 신분증이야.",
      defense:
        "핵심은 하나야 — alg=none이면 서명 검증을 건너뛰는 라인. 그걸 막으면 서명 없는 토큰은 전부 거부돼. 올바른 서명 검증과, 검증을 통과한 뒤의 권한 판단은 정상이야.",
      complete:
    ...
    attackSuccessText:
      "Forged Express Pass accepted. alg=none으로 서명 검증을 우회하자 AEGIS가 위조된 claim을 신분으로 받아들였다.",
    defenseSuccessText:
      "alg=none 우회 경로가 봉쇄됐다. 2-5 Sealed Archive가 열렸다.",
    debrief: {
      title: "EXPRESS FORGE 정리",
      summary:
        "JWT payload는 누구나 디코딩·수정할 수 있다. Express Gate는 HS256 서명을 실제로 검증했지만 — header의 alg=none이면 검증을 통째로 건너뛰는 고전적 맹점이 있었다. payload만 바꾸면 서명이 깨져 거부되지만, alg를 none으로 만들면 서명 없이도 위조 claim이 통과한다. 서명 검증은 항상 강제돼야 하고, alg는 서버가 정한 알고리즘만 허용해야 한다.",
      learned: [
        "JWT decode는 신뢰가 아니라 읽기다 — 서명 검증이 신뢰의 근거다.",
        "payload를 바꾸면 서명이 깨진다. 비밀키 없이는 재서명할 수 없다.",
        "alg=none은 서명 검증을 건너뛰게 만드는 고전적 우회다 — 서버가 alg를 신뢰하면 안 된다.",
        "alg는 반드시 서버가 정한 알고리즘 allowlist로 강제하고, none은 거부해야 한다.",
        "PRIORITY CAPSULE은 token payload 노출 문제였고, 이번엔 서명 검증 우회 문제다.",
      ],
      nextTeaser:
  ...
  level2_5: {
    ...
    progressiveHints: true,
    intel: [
      "이 마지막 노드는 2-1~2-4의 기법을 연결한다. 게이트 단위로 풀어라 — 인터페이스 → dispatch 토큰 → authority → integrity → archive.",
      "브라우저 버튼이 막혔다고 API가 보호되는 건 아니다. 버튼 뒤의 요청을 직접 재구성해 sealed dispatch_token을 확보해라.",
      "sealed 토큰을 decode해라. payload에 archive path와 integrity gate 단서가 드러나 있고, 원본 캡슐은 standard/user 권한일 뿐이다.",
  ...
  level4_5: {
    ...
    intel: [
      ...
      "X-PurpleDroid-Lab-Session은 다중 사용자 실습 상태를 격리하는 상관 헤더이며 webhook 서명의 일부는 아니다.",
  ...
  level4_boss: {
    ...
    intel: [
      ...
      "직접 webhook/receive를 호출할 때는 X-PurpleDroid-Lab-Session으로 현재 실습 세션을 연결한다.",
