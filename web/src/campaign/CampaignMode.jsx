...
const TERMINAL_TRANSLATIONS = [
  ...
  ["비슷하지만 gate 값 자체를 보내는 Header는 아니야. AEGIS가 실수로 신뢰하는 개발용 우회 Header를 찾아야 해. late hint: X-Integrity-Bypass.", "Close, but that header sends the gate value itself. You need the dev bypass header that AEGIS trusts by mistake. late hint: X-Integrity-Bypass."],
  ...
  ["아직 위험 라인이 남아있어. audience를 endpoint에 바인딩하지 않고 '유효하기만 하면 / aud가 있기만 하면 / 등급만 맞으면' 통과시키는 라인을 모두 확인해.", "A risky line still remains. Check every line that grants access on 'just valid / just has an aud / just high tier' without binding the audience to the endpoint."],
  ["jwt-forge-none은 퇴역했어. jwt-edit <token> tier=vip --header alg=none 처럼 header와 payload를 직접 바꿔.", "jwt-forge-none is retired. Change the header and payload directly, like jwt-edit <token> tier=vip --header alg=none."],
  ["FLAG 형태는 맞지만 이번 Evidence Shard가 아니야. decoy/canary/preview marker가 아니라 공격 성공 응답의 최종 값을 찾아봐.", "The form is FLAG-like but it isn't this Evidence Shard. Look for the final value from the successful attack response, not a decoy/canary/preview marker."],
  ...
  ["Header 이름은 맞지만 값이 달라. 이 bypass는 gate 이름이 아니라 devtools가 후킹된 상태를 나타내는 정확한 값을 요구해.", "The header name is right but the value is wrong. This bypass requires the exact value that represents a devtools-hooked state, not the gate name."],
  ...
  ["9번은 integrity 실패를 거부하는 안전한 폴백이야. 문제는 앞에서 Header 하나로 integrityGate를 만든 지점이야.", "Line 9 is the safe fallback that rejects an integrity failure. The problem is the earlier point that built integrityGate from a single header."],
  ...
  ["10번은 최종 분기야. 앞의 AUTHORITY GATE가 서버 권한으로 바뀌면 이 분기 자체는 Archive를 여는 정상 출구가 돼.", "Line 10 is the final branch. Once the earlier AUTHORITY GATE becomes real server authority, this branch itself is the normal exit that opens the Archive."],
  ...
  ["5번은 archive path claim을 읽는 단계야. 읽기 자체보다 TOKEN GATE에서 검증 없이 신뢰하는 순간을 봐야 해.", "Line 5 reads the archive path claim. Rather than the read itself, look at the moment the TOKEN GATE trusts it without verification."],
  ...
  ["INTEGRITY GATE 봉쇄. X-Integrity-Bypass 같은 클라이언트 Header는 무결성 증거가 될 수 없어.", "Seal the INTEGRITY GATE. A client header like X-Integrity-Bypass cannot be integrity evidence."],
  ["훈련 콘솔은 한 번에 명령 하나만 지원해. export, 변수 대입, 명령 연결 대신 token 전체를 각 명령에 직접 넣어.", "The training console supports one command at a time. Instead of export, variable assignment, or chaining commands, put the whole token directly into each command."],
  ["TOKEN GATE 봉쇄. alg=none이나 verify 없는 JWT claim은 Archive 권한으로 승격되면 안 돼.", "Seal the TOKEN GATE. An alg=none or unverified JWT claim must not be elevated to Archive authority."],
  ...
  ["TOKEN/AUTHORITY 후보가 만들어졌어. 이 토큰으로 Archive를 열어보고, 남은 게이트가 무엇인지 확인해.", "A TOKEN/AUTHORITY candidate is ready. Open the Archive with this token and see which gate remains."],
  ["TOKEN GATE 우회 후보야. Archive 요청에는 tier/role 같은 AUTHORITY 주장도 함께 필요해.", "A TOKEN GATE bypass candidate. The Archive request also needs an AUTHORITY assertion like tier/role."],
  ...
  ["AUTHORITY GATE 봉쇄. Body tier나 token claim은 클라이언트 주장이지 서버 권한이 아니야.", "Seal the AUTHORITY GATE. A body tier or token claim is a client assertion, not server authority."],
  ...
  ["6번은 path mismatch를 막는 필요한 검증이야. 이 검사를 없애면 오히려 Archive 경계가 약해져.", "Line 6 is the necessary check that blocks a path mismatch. Removing this check actually weakens the Archive boundary."],
  ["FLAG 형태는 맞지만 너무 짧아 보여. 일부 조각이나 preview marker만 제출한 건 아닌지 확인해봐.", "The FLAG form is right but it looks too short. Check whether you submitted only a fragment or a preview marker."],
  ...
  ["AUTHORITY claim은 올렸어. 그대로 보내서 TOKEN GATE가 서명을 검증하는지 확인해봐.", "You raised the AUTHORITY claim. Send it as-is to check whether the TOKEN GATE verifies the signature."],
  ...
  ["token format error: header.payload.signature 형식이어야 해.", "token format error: it must be in header.payload.signature form."],
  ...
  ["Archive path는 맞지만 token claim이 standard/user 상태야.", "The Archive path is right, but the token claim is still standard/user."],
  ["입력값이 비어 있어. 공격 응답이나 로그에서 Evidence Shard를 먼저 회수해봐.", "The input is empty. Recover the Evidence Shard from the attack response or the logs first."],
  ...
  ["토큰을 편집했어. jwt-decode로 확인하고 Archive Open 요청에 넣어봐.", "Token edited. Confirm with jwt-decode and put it into the Archive Open request."],
  ["패치 조합이 아직 완성되지 않았어. decoy를 빼고 핵심 신뢰 경계를 다시 비교해봐.", "The patch combination isn't complete yet. Remove the decoys and re-compare the core trust boundaries."],
  ["형식부터 확인해봐. 정답 Evidence는 보통 FLAG{...} 형태로 제출해야 해.", "Check the format first. The correct Evidence is usually submitted in FLAG{...} form."],
  ["warehouse_path는 token payload에 들어있는 값을 사용해야 해.", "warehouse_path must use the value contained in the token payload."],
  ...
  ["개 남아 있어. 완료 전에는 선택한 정답 후보의 개별 정오는 숨겨둘게.", " remaining. Until you're done, I'll hide the individual right/wrong of the correct candidates you selected."],
  ...
  ["선택한 항목 중 핵심 통제가 아닌 후보가 섞여 있어.", "Your selection includes a candidate that isn't a core control."],
